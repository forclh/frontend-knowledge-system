...
- **网络通信**
  ...
  a元素的跳转；加载css、js、图片等；AJAX等等，页面的源与访问的源不同
...
CORS将请求分为两类：==简单请求（simple request）==和==预检请求（preflight request）==。
...
- **头部字段**满足CORS安全规范，详见 [W3C](https://fetch.spec.whatwg.org/#cors-safelisted-request-header)
...
在跨域访问时，JS只能拿到一些最基本的响应头，如：Cache-Control、Content-Language、Content-Type、Expires、Last-Modified、Pragma，如果要访问其他头，则**需要服务器设置本响应头**。
